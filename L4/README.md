...
*There are over 30 computers all listed 12 cores.
...
Valgrind shows 32000 bytes still reachable a bunch of times, which is 8000 times 4 the size of the malloc array. Each process must be getting a copy of the main heap and its only cleared once?
...
Fork child processes share the same address space of parent whereas fork processes have separate memory space(but copy of parent) so color array can be accessed from every process with fork. Also vfork does not need wait.
