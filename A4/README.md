...
- just a couple the youtube video Vidoe made was pretty straightforward explanation of locking. the lab took many many hours though so by the time i finished the lab i had some idea what i was doing :)
- Did you collaborate with any other students/TAs/Professors? just alex to ask general questions
- Did you use any external resources? (Cite them below)
  - tbd
  - tbd
- (Optional) What was your favorite part of the assignment? this assignment was kind of cool :)
...
*my understanding from ta hours is that starvation would occur if you never UNLOCK... the program would starve because the other artists will be waiting for the prior artist threads to be unlocked. I ran without unlocking and it was mostly white because of the starvtion. This is different from a data race which can be caused by no locks and would lead to overlap as threads queue up and then paint over each other on some tiles.
...
I learned about calloc and how for its two parameters it can take in the number of elements and the size of the data type. Also calloc initializes each space to 0 by default instead of garbage data. Also realloc, which can change the size of dynamic memory -- I didnt know that you can even do that but I see it allocates new memory and then in some circumstances the old dynamic memory and new will be in one contiguous block (possibly leading to old block being deallocated). Also id never really seen malloc being used in a situation where you get information during runtime and dont know the size untill a point during runtime. I think this is because in class we havent really used scanf with c which makes situations where you dont know the size till after the scan :)



describing what you got caught up on. If you were caught up on 100% of things, write a few sentences about what new things you sat down and learned for 1-2 hours.
